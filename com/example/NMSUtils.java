...
public class NMSUtils{
    ...
    public static void registerEntity(final int id, final String name, final Type type,
            final Class<?> customClass, final Biome... biomes) {
        ...
        final Object key = mkClass.getConstructor(String.class).newInstance(name);
        
        Object etb = etClass.getField("b").get(null);
        etb.getClass().getMethod("a", int.class, mkClass, Class.class).invoke(etb, id, key, customClass);
        
        
        if (!((List) etClass.getField("d").get(null)).contains(key)) {
        	((List) etClass.getField("d").get(null)).add(key);
        }
    	} catch (Exception e) {
    		e.printStackTrace();
		}
        ...
        try {
            ...
            for (Biome biome : biomes) {
                final Object[] array = biome.getNMSBiomeArray();
                for (Object base : array) {
                    final List list = (List) field.get(base);
                    for (Object meta : list) {
                    	Object b = meta.getClass().getField("b").get(meta);
                        if (b == type.getNMSClass()) {
                            meta.getClass().getField("b").set(meta, customClass);
                            break;
                        }
                    }
                }
    ...
    public enum Biome {
        ...
        public Object asNMSBiome() {
            return this.biome;
        }
 
        public Object[] getNMSBiomeArray() {
            return this.biomes;
        }
        ...
        static {
            ...
            Class<?> bbaseClass = ReflectionUtils.getRefClass("{nms}.BiomeBase");
            for (Biome biome : values) {
            	Object registryId;
				try {
					registryId = bbaseClass.getField("REGISTRY_ID").get(null);
				
            	Method ridam = registryId.getClass().getMethod("a", bbaseClass);
                ID_LOOKUP_TABLE[(int) ridam.invoke(registryId, biome.biome)] = biome;
				} catch (Exception e) {
					e.printStackTrace();
				}
            }
