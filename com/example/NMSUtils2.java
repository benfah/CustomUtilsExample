...
public class NMSUtils2 {
     ...
     */
    public static void registerEntity(final Type type, final Class<?> customClass, boolean biomes) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static void registerEntity(final String name, final com.example.NMSUtils.Type bat, final Class<?> class1,
            boolean biomes) {
        final MinecraftKey key = new MinecraftKey(name);
        EntityTypes.b.a(bat.getId(), key, (Class<? extends Entity>) class1);
        if (!EntityTypes.d.contains(key)) {
            EntityTypes.d.add(key);
        }
        if (!biomes || bat.isSpecial()) {
            return;
        }
        final Field field;
        if ((field = bat.getMeta().getField()) == null) {
            return;
        }
        try {
            field.setAccessible(true);
            for (BiomeBase base : NMSUtils2.BIOMES) {
                final List<BiomeMeta> list = (List<BiomeMeta>) field.get(base);
                for (BiomeMeta meta : list) {
                    if (meta.b == bat.getNMSClass()) {
                        meta.b = (Class<? extends EntityInsentient>) class1;
                        break;
                    }
                }
            }
    ...
    public enum Type {
        ...
        private Type(final int id, final String name, Class<? extends Entity> nmsClazz, final MobMeta meta,
